...
*   **v2.3 (2026-08-13):** P10 Round 12 Governance Gate remediation: eliminated `numpy` dependency from `reproduce.py` in favor of a pure Python Standard Library linear interpolation percentile function, maintaining strict alignment with the Zero-Dependency policy in `versions.md` §3 (confirmed exact SHA-256 hash preservation `e141a998...` for `metrics.json` with 0 numeric diff); synchronized `README.md` to eliminate stale `audit_metrics.json` references, corrected non-English terminology, updated prerequisite declarations to Python 3.8+ Standard Library only, and clarified the reserved Concept DOI `10.5281/zenodo.21254253` pre-registered for Zenodo deposit.
